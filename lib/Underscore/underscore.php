...
class __
{
    ...
    public function invoke($collection = null, $function_name = null, $arguments = null)
    {
        ...
        $result = (empty($arguments)) ? array_map($function_name, (array)$collection) : array_map(
            $function_name,
            (array)$collection,
            $arguments
        );
    ...
    public function union($array = null)
    {
        ...
        return self::_wrap(
            $__->flatten(array_values(array_unique(call_user_func_array('array_merge', $arrays), SORT_REGULAR)))
        );
    }
    ...
    public function sortedIndex($collection = null, $value = null, $iterator = null)
    {
        ...
        while (count($collection) > 1) {
            ...
            $collection = ($calculated_value < $midpoint_calculated_value) ? array_slice(
                $collection,
                0,
                $midpoint,
                true
            ) : array_slice($collection, $midpoint, null, true);
        }
    ...
    public function template($code = null, $context = null)
    {
        ...
        $return = self::_wrap(
            function ($context = null) use ($code, $class_name) {
                ...
                if (count($vars) > 0) {
                    foreach ($vars as $var) {
                        $echo = $class_name::TEMPLATE_OPEN_TAG . ' echo htmlentities(' . trim(
                                $var[1]
                            ) . '); ' . $class_name::TEMPLATE_CLOSE_TAG;
                ...
                if (count($vars) > 0) {
                    foreach ($vars as $var) {
                        $echo = $class_name::TEMPLATE_OPEN_TAG . ' echo ' . trim(
                                $var[1]
                            ) . '; ' . $class_name::TEMPLATE_CLOSE_TAG;
                        $code = str_replace($var[0], $echo, $code);
                    }
        ...
        return self::_wrap(
            ((isset($this) && isset($this->_wrapped) && $this->_wrapped) || !is_null($context)) ? $return(
                $context
            ) : $return
        );
    }
    ...
    public function memoize($function = null, $hashFunction = null)
    {
        ...
        return self::_wrap(
            function () use ($function, &$_instance, $hashFunction) {
    ...
    public function throttle($function = null, $wait = null)
    {
        ...
        return self::_wrap(
            function () use ($function, $wait, &$_instance) {
                ...
                $ready_to_call = (!array_key_exists(
                        $key,
                        $_instance->_throttled
                    ) || $microtime >= $_instance->_throttled[$key]);
                if ($ready_to_call) {
    ...
    public function once($function = null)
    {
        ...
        return self::_wrap(
            function () use ($function, &$_instance) {
