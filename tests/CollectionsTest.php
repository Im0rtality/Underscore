...
class UnderscoreCollectionsTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testEach()
    {
        ...
        __::each(
            array(1, 2, 3),
            function ($num) use (&$answers, $context) {
        ...
        __::each(
            $obj,
            function ($value, $key) use (&$answers) {
                $answers[] = $key;
            }
        );
        ...
        __::each(
            array(1, 2, 3),
            function ($num, $index, $arr) use (&$answer) {
        ...
        __(array(1, 2, 3))->each(
            function ($num, $i) use ($test) {
                $test->assertEquals(
                    $num,
                    $i + 1,
                    'each iterators provide value and iteration count within OO-style call'
                );
            }
        );
        ...
        __::each(
            array(1, 2, 3),
            function ($num, $index) use ($multiplier, &$str) {
    ...
    public function testFilter()
    {
        ...
        // extra
        $odds = __(array(1, 2, 3, 4, 5, 6))->filter(
            function ($num) {
                return $num % 2 !== 0;
            }
        );
        ...
        $this->assertEquals(
            __::filter(array(1, 3, 5), $iterator),
            __::select(array(1, 3, 5), $iterator),
            'alias works'
        );
    ...
    public function testReject()
    {
        ...
        // extra
        $evens = __(array(1, 2, 3, 4, 5, 6))->reject(
            function ($num) {
                return $num % 2 !== 0;
            }
        );
    ...
    public function testInclud()
    {
        ...
        $this->assertTrue(
            __::contains((object)array('moe' => 1, 'larry' => 3, 'curly' => 9), 3),
            '__::includ on objects checks their values'
        );
    ...
    public function testReduceRight()
    {
        ...
        // extra
        $list = __(array('moe', 'curly', 'larry'))->reduceRight(
            function ($memo, $str) {
                return $memo . $str;
            },
            ''
    ...
    public function testSortBy()
    {
        ...
        $this->assertEquals(
            array($stooges[2], $stooges[1], $stooges[0]),
            __::sortBy(
                $stooges,
                function ($stooge) {
                    return $stooge['name'];
    ...
    public function testGroupBy()
    {
        ...
        // extra
        $parity = __(array(1, 2, 3, 4, 5, 6))->groupBy(
            function ($num) {
                return $num % 2;
            }
        );
        $this->assertEquals(
            array(array(2, 4, 6), array(1, 3, 5)),
            $parity,
            'created a group for each value using OO-style call'
        );
